...
**Status:** Revision-5 scientific contract. The executable Python model checks the bounded instances reported in the paper. A positively reviewed Lean 4 finite-carrier development now machine-checks the canonical abstract lifecycle, including computed Fork/Restore targets, checked fragment transfer, distinct Merge modes, and full trace induction. Boundary I/II and a production-runtime refinement remain separate obligations.
...
The Lean 4 development defines finite typed vectors, downward-closed durability families, a structural claim partition with distinct unissued/terminal states, executable AC admission, computed restriction, exact Prepare/cleanup, stable tickets and receipts, terminal/epoch monotonicity, labeled trace simulation, effect coverage, and a conditional trace-solvency theorem. It additionally computes exact choice/parallel Fork and replacing/live Restore targets, checks preallocated-unissued fragment transfer and coordinatewise fiber conservation, and separates simulation-certified from direct-admission Merge. A single authoritative Step and trace closure cover this complete abstract lifecycle.

The independent result review is positive. Canonical target WF, active-support exactness, and AC are derived from source invariants, exact builders, and source-local Boolean checks; canonical AC directly uses the checked fiber-conservation theorem. A clean 755-job build, source/axiom audit, and fresh kernel replay pass. The exact scope remains finite: dynamic identifier-space extension, issuer approval, refined effect binding, Boundaries I/II, complete mediation, aggregate sink truthfulness, and a real runtime adapter are not mechanized.
