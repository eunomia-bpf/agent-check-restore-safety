...
\end{aligned}
\]
Here \(\mathsf{Removed}_{H,u}\) contains the typed losing-arm outcomes for \(\mathsf{MergeSelect}\) or the current-role outcomes of a pending certified \(\mathsf{RestoreReplace}\).
% 此处 \(\mathsf{Removed}_{H,u}\) 包含 \(\mathsf{MergeSelect}\) 的类型化失败分支结果，或待处理且经认证的 \(\mathsf{RestoreReplace}\) 的当前角色结果。
...
The projection \(\pi_{\neg rc}\) analogously deletes receipt--effect-identity incidence and every path that reconstructs it, including receipt--creator, cursor--receipt, and archived creator--effect-identity paths, while retaining the receipt, creator, occurrence, and effect-identity marginals.
% 投影 \(\pi_{\neg rc}\) 类似地删除回执--效果标识关联以及每条可重建该关联的路径，包括回执--创建者、游标--回执和归档创建者--效果标识路径，同时保留回执、创建者、发生项和效果标识边际记录。
...
If \([\mathsf{Ans}(V_0,q)]_{\cong}\ne[\mathsf{Ans}(V_1,q)]_{\cong}\), then every exact information map \(f\) satisfies \(f(V_0)\not\cong_q f(V_1)\).
% 若 \([\mathsf{Ans}(V_0,q)]_{\cong}\ne[\mathsf{Ans}(V_1,q)]_{\cong}\)，则每个精确信息映射 \(f\) 都满足 \(f(V_0)\not\cong_q f(V_1)\)。
...
\begin{proof}
Assume toward contradiction that \(f(V_0)\cong_q f(V_1)\).
% 反设 \(f(V_0)\cong_q f(V_1)\)。
The joint decoder inputs \(\langle f(V_0),q\rangle_{\cong}\) and \(\langle f(V_1),q\rangle_{\cong}\) then coincide.
% 此时，联合解码器输入 \(\langle f(V_0),q\rangle_{\cong}\) 与 \(\langle f(V_1),q\rangle_{\cong}\) 相同。
...
\(T^0=b_x{:}X\oplus_g^{\rm open}b_y{:}Y\),
\(T^1=b_x{:}X\parallel_g b_y{:}Y\); otherwise equal. &
...
\(\ell(d_0)=\ell(d_1)=a\). &
\(X\parallel Y_k\): one \Fresh, accept;
two \Fresh reject \(aa\).\\
\(\mathbf E\) &
...
% \(\pi_{\neg rc}\)：回执--效果标识关联
For a separate pair, retain \(q_{\rm cell}(x)=d\) and effect identities \(d,e\) labeled \(a\), but let the archived creator bind \(p\mapsto d\) in \(V_{\neg rc}^0\) and \(p\mapsto e\) in \(V_{\neg rc}^1\).
% 对于另一对状态，保持 \(q_{\rm cell}(x)=d\) 以及以 \(a\) 标记的效果标识 \(d,e\)，但令归档创建者在 \(V_{\neg rc}^0\) 中绑定 \(p\mapsto d\)，在 \(V_{\neg rc}^1\) 中绑定 \(p\mapsto e\)。
...
Here the only differing occurrence--effect-identity row belongs to the archived receipt creator; \(\pi_{\neg rc}\) explicitly deletes that reconstructing path together with receipt--effect-identity incidence, leaving isomorphic retained views with equal receipt, occurrence, and effect-identity marginals.
% 此处唯一不同的发生项--效果标识记录属于已归档的回执创建者；\(\pi_{\neg rc}\) 会连同回执--效果标识关联显式删除这条重建路径，从而留下具有相等回执、发生项和效果标识边际记录的同构保留视图。
...
% 本证明中的任何步骤都不推断自然语言契约、不提供公平性、不只禁用单个域的一部分，也不把本地排队请求释放强化为远程恰好一次执行。
