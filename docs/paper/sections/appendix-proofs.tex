...
Fix a well-formed safety-check instance \(\Theta=(\kappa,H,\Delta,\mathsf{out},\mathcal A,u)\) for which the history judgment derives \(\delta=(H^+,\mathcal C,\eta^-,\eta^+)\).
% 固定一个良构的安全检查实例 \(\Theta=(\kappa,H,\Delta,\mathsf{out},\mathcal A,u)\)，其历史判定派生出 \(\delta=(H^+,\mathcal C,\eta^-,\eta^+)\)。
...
\end{aligned}
\]
Here \(\mathsf{Dec}_{H,u}\) contains the typed losing-arm outcomes for \(\mathsf{MergeSelect}\) or the current-role outcomes of a pending certified \(\mathsf{RestoreReplace}\).
% 此处 \(\mathsf{Dec}_{H,u}\) 包含 \(\mathsf{MergeSelect}\) 的类型化失败分支结果，或待处理且经认证的 \(\mathsf{RestoreReplace}\) 的当前角色结果。
\(\mathsf{Done}_{H,u}\) contains the completed current role of \(\mathsf{RestoreReplace}\).
% \(\mathsf{Done}_{H,u}\) 包含 \(\mathsf{RestoreReplace}\) 已完成的当前角色。
Both sets are empty for every other row and for live extension.
...
\path{CompilationBridge.compiler_admit_exists_secure_install} constructs a valid installed cut from an accepted edit, and its converse declarations recover acceptance from any valid install candidate.
% \path{CompilationBridge.compiler_admit_exists_secure_install} 从被接受的编辑构造有效安装切点，而其逆向声明从任意有效安装候选恢复接受判定。
...
% 每条外键路径依赖其目标。
Authenticated gate and handle bytes depend on their \(\mathbf I\) slot and \(\mathbf C\) epoch and owner.
% 认证门与句柄字节依赖其 \(\mathbf I\) 槽位以及 \(\mathbf C\) 纪元和所有者。
Receipt reconstruction and the authority word depend on the \(\mathbf E\) receipt and cursor rows together with their \(\mathbf I\) cells and labels.
% 回执重建与权限词依赖 \(\mathbf E\) 的回执与游标记录以及对应的 \(\mathbf I\) 单元与标签。
Target certificates, cut seals, and digests depend on every encoded base field in their payload.
...
% 每个分离证明都遵循同一方法。
Starting from a common checked genesis, we vary exactly one security-state factor, keep the visible projection and literal query fixed, and obtain opposite correct answers.
% 从共同的经检查起源状态出发，我们只改变一个安全状态因子，保持可见投影与字面查询不变，并得到相反的正确答案。
...
The cut origin is the checked root derivation, while every later cut-changing transition records an execution-edit derivation or an authenticated live extension.
% 该切点的来源是经检查的根推导，而此后每个改变切点的转移都记录一种执行编辑推导或一次经认证的在线扩展。
\end{lemma}
...
Only initial installation creates a root origin, and every successful later installation stores its checked execution-edit or live-extension derivation.
% 只有初始安装会创建根来源，而此后每次成功安装都会存储其经检查的执行编辑或在线扩展推导。
\end{proof}
...
\begin{proof}
\Cref{lem:app-boot} gives the common genesis and valid root installation, while the explicit traces above use only modeled Save, protected-use, and execution-edit transitions.
% \Cref{lem:app-boot} 给出共同起源状态和有效根安装，而上述显式轨迹只使用已建模的 Save、受保护使用和执行编辑转移。
...
Initial registration occurs before the root safety check; later additive schema and policy growth is the authenticated live-extension history edge listed below.
% 初始注册发生在根安全检查之前；后续仅追加的模式与策略增长由下表所列的经认证在线扩展历史边完成。
...
This argument applies uniformly to all six execution edits and authenticated live extension.
% 该论证一致地适用于全部六种执行编辑与经认证的在线扩展。
