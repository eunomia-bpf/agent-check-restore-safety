...
two \Fresh reject \(aa\).\\
\(\mathbf E\) &
\(q_{\mathbf E}=u_R(x)\) after safe empty-root/ForkChoice/Save/\Fresh;
...
A registered slice \(R\) compiled from a finite typed Agent workflow \(\mathcal W\) contains only finite contracts, schemas, policy, lowering data, and authenticated occurrence, cell, invocation, lineage, scope, and label rows.
% 从有限类型化 Agent 工作流 \(\mathcal W\) 编译得到的注册切片 \(R\) 只包含有限契约、模式、策略、降低数据，以及经认证的发生项、单元、调用、谱系、作用域和标签行。
...
For finite \(\mathcal W,R\), \(\mathsf{CheckReg}(\mathcal W,R)\) accepts iff the registered \(\lambda\) witnesses \(\mathcal W\sqsubseteq_\lambda R\) in \cref{def:registration-refinement}.
% 对于有限的 \(\mathcal W,R\)，当且仅当已注册的 \(\lambda\) 见证 \cref{def:registration-refinement} 中的 \(\mathcal W\sqsubseteq_\lambda R\) 时，\(\mathsf{CheckReg}(\mathcal W,R)\) 才接受。
...
\end{lemma}
\begin{proof}
The finite checker enumerates the boundary classes of \(\mathcal W\) and \(\mathsf{BRuns}(R)\) and directly decides outcome/site incidence, order, cell attestation, total unique lowering, raw-bypass exclusion, and prefix receipt commutation.
% 有限检查器枚举 \(\mathcal W\) 的边界等价类与 \(\mathsf{BRuns}(R)\)，并直接判定结果/位置关联关系、次序、单元证明、全定义唯一降低、原始绕过排除与前缀回执交换性。
...
% \(\mathbf E\)：回执--单元关联
Let \(C,X\) be singleton contracts with occurrences \(c,x\), let \(q_{\rm cell}(x)=d\), and label cells \(d,e\) by \(a\).
% 令 \(C,X\) 为分别具有发生项 \(c,x\) 的单元素契约，令 \(q_{\rm cell}(x)=d\)，并以 \(a\) 标记单元 \(d,e\)。
From the safe \(\mathbf 1\) root, a common Fork schema installs \(b_L{:}C\oplus_g^{\mathsf{open}}b_R{:}X\), because each outcome again redeems one cell.
% 从安全的 \(\mathbf 1\) 根出发，公共 Fork 模式安装 \(b_L{:}C\oplus_g^{\mathsf{open}}b_R{:}X\)，因为每个结果仍然只兑付一个单元。
Save \(b_R\), then use \(c\) to select and complete the left arm and create receipt \(p\).
% 保存 \(b_R\)，随后使用 \(c\) 选中并完成左侧分支，同时创建回执 \(p\)。
The views have identical marginals and current authority word \(a\), but the archived creator binds \(p\mapsto d\) in \(V_{\mathbf E}^0\) and \(p\mapsto e\) in \(V_{\mathbf E}^1\).
% 两个视图具有相同的边际记录与当前权限词 \(a\)，但归档创建者在 \(V_{\mathbf E}^0\) 中绑定 \(p\mapsto d\)，在 \(V_{\mathbf E}^1\) 中绑定 \(p\mapsto e\)。
Issue \(q_{\mathbf E}=\mathsf{RestoreReplace}(b_L,k,\sigma_{\mathbf E})\).
% 发出 \(q_{\mathbf E}=\mathsf{RestoreReplace}(b_L,k,\sigma_{\mathbf E})\)。
The completed current continuation is eligible for retirement.
% 已完成的当前延续可被退役。
The cloned \(x_k\) is \(\Alias(x_k,d)\) in \(V_{\mathbf E}^0\), retaining authority word \(a\), but \(\Fresh(x_k,d)\) in \(V_{\mathbf E}^1\), producing forbidden word \(aa\).
% 克隆的 \(x_k\) 在 \(V_{\mathbf E}^0\) 中为 \(\Alias(x_k,d)\)，从而保持权限词 \(a\)，但在 \(V_{\mathbf E}^1\) 中为 \(\Fresh(x_k,d)\)，从而产生被禁止的权限词 \(aa\)。
The archived creator path is receipt-only, so deleting \(\mathbf E\), or receipt--cell incidence and every reconstructing path, leaves isomorphic equal marginals under both \(\pi_{\{\mathbf P,\mathbf I,\mathbf C\}}\) and \(\pi_{\neg rc}\).
% 归档创建者路径仅供回执使用，因此删除 \(\mathbf E\)，或删除回执--单元关联及每条重建路径，会在 \(\pi_{\{\mathbf P,\mathbf I,\mathbf C\}}\) 与 \(\pi_{\neg rc}\) 下留下同构且相等的边际记录。
...
Open choice enables both Save steps, and the \(\mathbf E\) monitor enables the stated left-arm \Fresh before that arm becomes a completed, still-addressable continuation.
% 开放选择启用两次 Save，而 \(\mathbf E\) 监控器会在左侧分支成为已完成但仍可寻址的延续之前启用所述左侧分支 \Fresh。
...
% 历史边
The ideal machine forms the same \(\Theta_I(u)\) and enables its history edge exactly when the schema judgment exists and \(\mathsf{Spec}(\Theta_I(u))\) is defined.
% 理想机形成相同的 \(\Theta_I(u)\)，并且当且仅当模式判定存在且 \(\mathsf{Spec}(\Theta_I(u))\) 有定义时启用其历史边。
...
% 回复、释放与静默步骤
An authenticated retry, bundle retrieval, or result delivery follows the same authenticated link on both sides and exposes the same \(\mathsf{Return}(t,[v]_{\cong})\).
% 经认证重试、包取回或结果交付在两侧沿相同的认证链接，并暴露相同的 \(\mathsf{Return}(t,[v]_{\cong})\)。
...
Arbitrary repetition of silent preload, failed installation, Save, or crash steps is ignored, which is precisely divergence-insensitive weak matching.
% 任意重复的静默预加载、失败安装、Save 或崩溃步骤均被忽略，这恰是发散不敏感弱匹配。
\end{proof}
