...
A registered slice \(R\) contains only finite contracts, schemas, policy, and authenticated occurrence, cell, invocation, lineage, scope, and label rows.
% 已注册切片 \(R\) 只包含有限契约、模式、策略，以及经认证的发生项、单元、调用、谱系、作用域和标签行。
...
The pre-slice relation has exactly a registration step \(\mathsf S_\emptyset\xrightarrow{\mathsf{register}(R,n)}\mathsf{Reg}(R,n)\), where \(n\) is a fresh private namespace, followed by checked root installation.
% 切片前关系恰好包含注册步骤 \(\mathsf S_\emptyset\xrightarrow{\mathsf{register}(R,n)}\mathsf{Reg}(R,n)\)，其中 \(n\) 是新鲜的私有命名空间，随后执行经检查的根安装。
...
% 自然引导
\label{lem:app-boot}
If \(R\)'s registered root contract and policy have a nonempty greatest realization, the registration and checked root-installation steps reach a finite state \(S_R\) with \(\mathsf{AgentSec}(S_R;c_R,\epsilon)\).
% 若 \(R\) 的已注册根契约与策略具有非空的最大实现，则注册与经检查的根安装步骤会到达一个满足 \(\mathsf{AgentSec}(S_R;c_R,\epsilon)\) 的有限状态 \(S_R\)。
The cut origin is the checked root derivation, while every later cut-changing transition records one of the six history derivations.
% 该切点的来源是经检查的根推导，而此后每个改变切点的转移都记录六种历史推导之一。
\end{lemma}

\begin{proof}
Registration contributes only \(R\) and \(n\), and deterministic allocation constructs every runtime identifier from them.
% 注册只提供 \(R\) 和 \(n\)，而确定性分配据此构造每个运行时标识。
...
Only initial installation creates a root origin, and every successful later installation stores its checked edit derivation.
% 只有初始安装会创建根来源，而此后每次成功安装都会存储其经检查的编辑推导。
\end{proof}
...
% \(\mathbf I\)：发生项--单元关联
Let the authenticated checkpoint contain the sequential target contract \(x\prec y\), let the current branch be schema-certified for exact retirement, and query \(q_{\mathbf I}=u_R(x\prec y)\).
% 令经认证检查点包含顺序目标契约 \(x\prec y\)，令当前分支由模式认证可被精确退役，并查询 \(q_{\mathbf I}=u_R(x\prec y)\)。
Both traces install the registered \(x\prec y\) root and execute \(\mathsf{Save}(k,b)\) before progress; the registered \(\mathsf{RestoreReplace}\) schema then authorizes exact retirement of the current continuation.
% 两条轨迹都安装已注册的 \(x\prec y\) 根，并在取得进展前执行 \(\mathsf{Save}(k,b)\)；随后，已注册的 \(\mathsf{RestoreReplace}\) 模式授权精确退役当前延续。
...
Both traces install a two-step root \(c\prec x\), resolve \(c\) by \Fresh, and save the residual singleton \(x\); \(c\) maps to \(d\) in \(V_{\mathbf E}^0\) and to \(e\) in \(V_{\mathbf E}^1\).
% 两条轨迹都安装两步根 \(c\prec x\)，通过 \Fresh 解析 \(c\)，并保存残余单元素 \(x\)；在 \(V_{\mathbf E}^0\) 中 \(c\) 映射到 \(d\)，而在 \(V_{\mathbf E}^1\) 中映射到 \(e\)。
...
Use the same schema-certified retirement condition and query \(q_{\mathbf E}=u_R(x)\).
% 使用相同的模式认证退役条件，并查询 \(q_{\mathbf E}=u_R(x)\)。
...
% \(\mathbf C\)：历史切点
Let \(V_{\mathbf C}^0\) and \(V_{\mathbf C}^1\) arise from alternative registrations of the same public slice under fresh private namespaces, followed by deterministic checked root installation.
% 令 \(V_{\mathbf C}^0\) 与 \(V_{\mathbf C}^1\) 由同一公开切片在不同新鲜私有命名空间下的备选注册产生，随后执行确定性的经检查根安装。
They have identical normalized \(\mathbf P,\mathbf I,\mathbf E\) relations but different private live epochs, view versions, gate ownership, and dependent seals and digests.
% 它们具有相同的规范化 \(\mathbf P,\mathbf I,\mathbf E\) 关系，但具有不同的私有活跃纪元、视图版本、门所有权以及依赖的封印和摘要。
...
A new registration is not an in-slice mutation: the system returns to the pre-slice registration phase and performs checked root admission before the listed rules apply.
% 新注册并非切片内变更：系统先返回切片前注册阶段并执行经检查的根准入，随后才应用表中规则。
