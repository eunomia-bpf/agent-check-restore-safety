...
A registered slice \(R\) compiled from a finite typed Agent workflow \(A\) contains only finite contracts, schemas, policy, and authenticated occurrence, cell, invocation, lineage, scope, and label rows.
% 从有限类型化 Agent 工作流 \(A\) 编译得到的注册切片 \(R\) 只包含有限契约、模式、策略，以及经认证的发生项、单元、调用、谱系、作用域和标签行。
...
The pre-slice relation has a registration step \(\mathsf S_\emptyset\xrightarrow{\mathsf{register}(A,R,n)}\mathsf{Reg}(R,n)\) exactly when \(\mathsf{CheckReg}(A,R)\) accepts and \(n\) is a fresh private namespace, followed by checked root installation.
% 当且仅当 \(\mathsf{CheckReg}(A,R)\) 接受且 \(n\) 是新鲜私有命名空间时，切片前关系才包含注册步骤 \(\mathsf S_\emptyset\xrightarrow{\mathsf{register}(A,R,n)}\mathsf{Reg}(R,n)\)，随后执行经检查的根安装。
...
For finite \(A,R\), \(\mathsf{CheckReg}(A,R)\) accepts iff \(A\sqsubseteq R\) satisfies \cref{def:registration-refinement}.
% 对于有限的 \(A,R\)，当且仅当 \(A\sqsubseteq R\) 满足 \cref{def:registration-refinement} 时，\(\mathsf{CheckReg}(A,R)\) 才接受。
On acceptance, source executions and registered executions correspond bijectively, and the correspondence preserves indexed outcomes, causal order, canonical invocation quotienting, receipt evolution, and the admission answer.
% 接受时，源执行与注册执行双射对应，并且该对应保持带索引结果、因果次序、规范调用商、回执演化与准入答案。
\end{lemma}
\begin{proof}
The finite checker directly decides the incidence, order, exact-retirement, cell-attestation, and boundary-mediation clauses.
% 有限检查器直接判定关联、次序、精确退役、单元证明与边界中介条款。
Canonical invocation equality induces exactly the registered cell quotient, and unique lowering maps each durably external source edge to one protected \Use while excluding a raw bypass.
% 规范调用相等性恰好诱导注册单元商，而唯一降低映射将每条持久外部源边映射到一次受保护 \Use，并排除原始绕过路径。
Induction over source prefixes and deterministic \(\Resolve\) proves matching \Fresh/\Alias receipt evolution; exact retirement makes compilation and pullback inverse on the surviving indexed outcomes.
% 对源前缀和确定性 \(\Resolve\) 作归纳可证明相匹配的 \Fresh/\Alias 回执演化；精确退役使编译与回拉在保留的带索引结果上互为逆映射。
The realization conditions and greatest-fixed-point operator are invariant under this bijection, which proves admission and greatest-monitor preservation.
% 实现条件与最大不动点算子在该双射下保持不变，从而证明准入与最大监控器保持。
\end{proof}

\begin{lemma}[Natural bootstrap]
% 自然引导
\label{lem:app-boot}
If \(\mathsf{CheckReg}(A,R)\) accepts and \(R\)'s registered root contract and policy have a nonempty greatest realization, registration and checked root installation reach a finite state \(S_R\) with \(\mathsf{AgentSec}(S_R;c_R,\epsilon)\).
% 若 \(\mathsf{CheckReg}(A,R)\) 接受且 \(R\) 的已注册根契约与策略具有非空的最大实现，则注册与经检查的根安装会到达一个满足 \(\mathsf{AgentSec}(S_R;c_R,\epsilon)\) 的有限状态 \(S_R\)。
...
Compile the same registered edit \(u_0\) at \(V_{\mathbf C}^0\) to obtain \(Z_0\), and submit the same literal query \(q_{\mathbf C}=\mathsf{Install}(u_0,Z_0)\) to both views.
% 在 \(V_{\mathbf C}^0\) 处编译同一个已注册编辑 \(u_0\) 得到 \(Z_0\)，并向两个视图提交同一个字面查询 \(q_{\mathbf C}=\mathsf{Install}(u_0,Z_0)\)。
