...
func bindTestJournal(t *testing.T, journal *Journal, schema int) {
	t.Helper()
	configDigest := ""
	if schema == JournalSchema {
		configDigest = testDigest("tool-configuration")
	}
	if err := journal.BindSchema(schema, configDigest); err != nil {
		t.Fatal(err)
	}
}
...
func TestJournalPersistsPreparedIdentityAndCompletedResponse(t *testing.T) {
	...
	bindTestJournal(t, journal, LegacyJournalSchema)
	if _, err := OpenJournal(path, "execution-journal"); err == nil {
		t.Fatal("a second process acquired the locked journal")
	}
	call, err := journal.Prepare(LegacyJournalSchema, "1", "1", testDigest("request-one"))
	if err != nil {
		t.Fatal(err)
	}
	...
	bindTestJournal(t, reopened, LegacyJournalSchema)
	defer reopened.Close()
	recorded, exists, err := reopened.Lookup(LegacyJournalSchema, "1", testDigest("request-one"))
	...
	second, err := reopened.Prepare(LegacyJournalSchema, "2", "2", testDigest("request-two"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestJournalStableIdentitySurvivesRPCChangeAndRejectsSchemaChange(t *testing.T) {
	path := filepath.Join(privateTestDirectory(t), "calls.jsonl")
	journal, err := OpenJournal(path, "stable-scope")
	if err != nil {
		t.Fatal(err)
	}
	bindTestJournal(t, journal, JournalSchema)
	key := journalCallKeyPrefix + testDigest("effect-A")
	call, err := journal.Prepare(JournalSchema, key, `2`, testDigest("request-A"))
	if err != nil {
		t.Fatal(err)
	}
	expectedID := "mcp-call-v2:12:stable-scope:" + testDigest("effect-A")
	if call.CallID != expectedID || call.Sequence != 1 {
		t.Fatalf("stable call=%+v", call)
	}
	response := []byte(`{"jsonrpc":"2.0","id":null,"result":{"ok":true}}`)
	if err := journal.Complete(call, response, false); err != nil {
		t.Fatal(err)
	}
	if err := journal.Close(); err != nil {
	...
	reopened, err := OpenJournal(path, "stable-scope")
	if err != nil {
		t.Fatal(err)
	}
	defer reopened.Close()
	if err := reopened.BindSchema(LegacyJournalSchema, ""); err == nil {
		t.Fatal("schema-2 journal reopened as positional schema")
	}
	if err := reopened.BindSchema(JournalSchema, testDigest("different-configuration")); err == nil {
		t.Fatal("schema-2 journal reopened with a different tool configuration")
	}
	bindTestJournal(t, reopened, JournalSchema)
	replayed, exists, err := reopened.Lookup(JournalSchema, key, testDigest("request-A"))
	if err != nil || !exists || !replayed.Completed || replayed.RPCID != "2" || string(replayed.Response) != string(response) {
		t.Fatalf("replayed=%+v exists=%t error=%v", replayed, exists, err)
	}
}
...
func TestJournalResumesPreparedCallAndRejectsDifferentWork(t *testing.T) {
	...
	bindTestJournal(t, journal, LegacyJournalSchema)
	prepared, err := journal.Prepare(LegacyJournalSchema, `"rpc-7"`, `"rpc-7"`, testDigest("original"))
	if err != nil {
		t.Fatal(err)
	}
	if err := journal.Close(); err != nil {
	...
	bindTestJournal(t, reopened, LegacyJournalSchema)
	defer reopened.Close()
	replayed, exists, err := reopened.Lookup(LegacyJournalSchema, `"rpc-7"`, testDigest("original"))
	...
	if _, _, err := reopened.Lookup(LegacyJournalSchema, `"rpc-7"`, testDigest("different")); err == nil {
		t.Fatal("pending JSON-RPC identity accepted different work")
	}
	if _, err := reopened.Prepare(LegacyJournalSchema, `"rpc-8"`, `"rpc-8"`, testDigest("later")); err == nil {
	...
	later, err := reopened.Prepare(LegacyJournalSchema, `"rpc-8"`, `"rpc-8"`, testDigest("later"))
...
func TestJournalPersistsExecutionFence(t *testing.T) {
	...
	bindTestJournal(t, journal, LegacyJournalSchema)
	call, err := journal.Prepare(LegacyJournalSchema, "1", "1", testDigest("uncertain"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := reopened.Prepare(LegacyJournalSchema, "2", "2", testDigest("must-not-run")); err == nil {
...
func TestJournalRejectsWrongExecutionMutationAndUnsafePaths(t *testing.T) {
	...
	bindTestJournal(t, journal, LegacyJournalSchema)
	call, err := journal.Prepare(LegacyJournalSchema, "1", "1", testDigest("work"))
	if err != nil {
		t.Fatal(err)
	}
