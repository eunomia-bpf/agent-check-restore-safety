...
func TestDecodeConfigStrictRoundTrip(t *testing.T) {
	...
	if got.Schema != want.Schema || got.SessionID != want.SessionID || got.CodexSHA256 != want.CodexSHA256 || got.StreamPort != want.StreamPort || got.ModelPort != want.ModelPort || got.PayloadDrive != want.PayloadDrive || strings.Join(got.Arguments, "\x00") != strings.Join(want.Arguments, "\x00") {
...
func TestDecodeConfigRejectsMissingUnknownDuplicateAndTrailing(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "duplicate", data: strings.Replace(string(valid), `"schema":1`, `"schema":1,"schema":1`, 1)},
...
func TestConfigValidationRejectsAuthorityExpansion(t *testing.T) {
	tests := []struct {
		...
		{name: "schema", mutate: func(c *Config) { c.Schema = 2 }},
