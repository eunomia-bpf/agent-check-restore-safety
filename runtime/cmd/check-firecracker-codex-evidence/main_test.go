...
func newFixture(t *testing.T) *fixture {
	...
	f.result = resultRecord{Schema: 1, Success: true, SessionID: session, CodexSHA256: codex.SHA256, RunnerSHA256: artifacts["runner"].SHA256, ArgumentsSHA256: hashBytes(mustMarshal(t, args)), ArgumentsEncoding: "compact-json-array", ArgumentsCount: len(args), WorkspaceMapping: workspaceMapping{Host: workspace, Guest: "/workspace"}, Artifacts: artifacts, SealedBootInputs: boot, SealedLoadInputs: load, Checkpoint: checkpointValue, RepositoryChange: repositoryChangeRecord{BaseRoot: repository.TreeRoot.String(), FinalRoot: finalRepository.TreeRoot.String(), OperationCount: len(repositoryDelta.Operations)}, Processes: []processRecord{g1, g3}, G1SIGKILLConfirmed: true, SnapshotLoadedPaused: true, RelayArmedBeforeResume: true, ToolReleasedAfterAttach: true, CompletedTimeNS: 2400}
