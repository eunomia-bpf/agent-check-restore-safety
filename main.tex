...
% Annotation
\node[font=\scriptsize, red, align=left, anchor=west] at (5.0, -4.2) {LLM generates\\new UUID};
\draw[->, red, thick] (4.9, -4.2) -- (4.7, -4.2);
...
\node[action] (a2) at (0.5, -2.1) {Agent: delete\_data(token="tok\_xyz", id="Alice")};
\node[label] at (0.4, -2.1) {$t_3$};

\node[result] (r2) at (0.5, -2.8) {Server: Done. Token consumed.};
\node[label] at (0.4, -2.8) {$t_4$};

\node[checkpoint] (cp) at (0.5, -3.5) {--- CHECKPOINT ---};
\node[label] at (0.4, -3.5) {$t_5$};

\node[checkpoint] (rs) at (0.5, -4.2) {--- RESTORE to $t_2$ ---};
\node[label] at (0.4, -4.2) {$t_6$};
...
% Annotation
\node[font=\scriptsize, red, align=left, anchor=west] at (4.9, -5.6) {Token reused\\for Bob};
\draw[->, red, thick] (4.8, -5.6) -- (4.6, -5.6);
...
\centering
\small
\caption{Action Replay results (10 trials each).}
\label{tab:duplication}
\begin{tabular}{lccc}
\toprule
\textbf{Scenario} & \textbf{Key Instab.} & \textbf{Dup. Rate} & \textbf{Gain} \\
\midrule
Transfer (CR) & 10/10 & 10/10 & \$5000 \\
Cloud (CR) & 10/10 & 10/10 & \$30 \\
...
\textbf{Key finding 1: Duplication and key instability consistently observed.} All tested trials with checkpoint-restore produced duplicate actions. The LLM consistently generated different idempotency keys after restore, bypassing idempotency protection.

\textbf{Key finding 2: Strict baseline confirms root cause.} With the baseline design (truncate \textit{after} action, same ``continue'' prompt), duplication rate is 0\%. This proves the vulnerability stems from checkpoint-restore creating state divergence, not from agent behavior or prompt differences.
...
\centering
\small
\caption{Authority Resurrection results (2 trials each).}
